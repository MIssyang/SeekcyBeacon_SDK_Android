		...
		SKYBeaconManager.getInstance().startScanService(
				new ScanServiceStateCallback() {
					...
					public void onServiceConnected() {
						// TODO Auto-generated method stub
						SKYBeaconManager.getInstance()
								.startRangingBeacons(null);
					}
				});

		SKYBeaconManager.getInstance().setRangingBeaconsListener(
				new RangingBeaconsListener() {

					@SuppressWarnings("rawtypes")
					@Override
					public void onRangedBeaconsMultiIDs(SKYRegion beaconRegion,
							List beaconMultiIDsList) {
						// TODO Auto-generated method stub
						for (int i = 0; i < beaconMultiIDsList.size(); i++) {
							iBeaconView beacon = new iBeaconView();
							beacon.mac = ((SKYBeaconMultiIDs) beaconMultiIDsList.get(i))
									.getDeviceAddress();
							beacon.isMultiIDs = true;
							beacon.detailInfo = "version: "
									+ String.valueOf(((SKYBeaconMultiIDs) beaconMultiIDsList
											.get(i)).getHardwareVersion())
									+ "."
									+ String.valueOf(((SKYBeaconMultiIDs) beaconMultiIDsList
											.get(i)).getFirmwareVersionMajor())
									+ "."
									+ String.valueOf(((SKYBeaconMultiIDs) beaconMultiIDsList
											.get(i)).getFirmwareVersionMinor());
							beacon.detailInfo += "\r\n";
							for (int j = 0; j < ((SKYBeaconMultiIDs) beaconMultiIDsList
									.get(i)).getBeaconList().size(); j++) {
								beacon.detailInfo += ((SKYBeacon) ((SKYBeaconMultiIDs) beaconMultiIDsList
										.get(i)).getBeaconList().get(j))
										.getDeviceAddress()
										+ "\r\n";
							}
					...
					public void onRangedBeacons(SKYRegion beaconRegion,
						...
						for (int i = 0; i < beaconList.size(); i++) {
							iBeaconView beacon = new iBeaconView();
							beacon.mac = ((SKYBeacon) beaconList.get(i))
									.getDeviceAddress();
							beacon.isMultiIDs = false;
							beacon.detailInfo = "version: "
									+ String.valueOf(((SKYBeacon) beaconList
											.get(i)).getHardwareVersion())
									+ "."
									+ String.valueOf(((SKYBeacon) beaconList
											.get(i)).getFirmwareVersionMajor())
